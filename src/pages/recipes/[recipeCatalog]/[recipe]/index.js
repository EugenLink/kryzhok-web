          ...
          <div>
            <div className={styles.backgroundWrapper}>
              <div className={styles.contentStyle}>
                <Image
                  src={`https://u1978287.isp.regruhosting.ru/recipes/photos/${data[0]}/logo.png`}
                  alt="Банер"
                  fill
...
export async function getServerSideProps(context) {
  ...
  const res = await fetch(
    `https://u1978287.isp.regruhosting.ru/recipes/getRecipe.php?id=${recipe}`
  );
  const data = await res.json();

  // Pass data to the page via props
  fetch(
    `https://u1978287.isp.regruhosting.ru/recipes/setView.php?id=${
      data[0]
    }&views=${+data[6] + 1}`
  );
  return { props: { data } };
